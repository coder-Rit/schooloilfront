...
export const get_all_recent_lectures = (givenData,howMuchOld) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.get(
      `https://schooloil-api.onrender.com/api/v1/user/getAllLectures/${id}/${from}/${to}`
    ); 
    console.log(data);
...
export const add_user_to_mark_attendace =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        `https://schooloil-api.onrender.com/api/v1/user/markMyAttendance/${updater}/${update}`
      );
      console.log(data);
      dispatch({
...
export const delete_created_lecture = (id, lectures) => async (dispatch) => {
   ...
   const { data } = await axios.delete(
    `https://schooloil-api.onrender.com/api/v1/user/lecture/${id}`
    );
